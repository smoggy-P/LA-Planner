 ...
 */
inline int update_trial_interval(double* x, double* fx, double* dx, double* y, double* fy, double* dy, double* t, double* ft,
    double* dt, const double tmin, const double tmax, int* brackt) {
  int bound;
...
inline int line_search_morethuente(int n, double* x, double* f, double* g, double* s, double* stp, const double* xp,
    const double* gp, const double* stpmin, const double* stpmax, callback_data_t* cd, const lbfgs_parameter_t* param) {
  int count = 0;
    ...
    */
    ...
    while (loop == 1) {
      ...
      if (cd.proc_stepbound) {
        ...
        if (step >= step_max) step = step_max / 2.0;
      }
