...
SDFMap::SDFMap(const bool isGlobal) : isGlobalMap_(isGlobal) {
}
...
void SDFMap::inputGlobalPointCloud(const pcl::PointCloud<pcl::PointXYZ>& global_points) {
  ...
  for (const auto& pt : global_points.points) {
    ...
    if (md_->flag_rayend_[vox_adr] != md_->raycast_num_) {
      md_->flag_rayend_[vox_adr] = md_->raycast_num_;
      caster_->input(pt_w, Eigen::Vector3d(0, 0, 0));  // Using global origin
      caster_->nextId(idx);
      while (caster_->nextId(idx)) setCacheOccupancy(toAddress(idx), 0);
    }
  }
...
bool SDFMap::checkObstacleBetweenPoints(const Eigen::Vector3d& start, const Eigen::Vector3d& end, RayCaster* caster) {
  RayCaster* ray_caster = (caster == nullptr) ? caster_.get() : caster;

  // Since that features are usually attached to the surface, raycast may lead to incorrect judgement
  // Here we use a relaxed detection criteria to avoid this situation

  ray_caster->input(start, end);
  Eigen::Vector3i idx;
  int occupied_count = 0;
  while (ray_caster->nextId(idx)) {
    if (getOccupancy(idx) == OCCUPIED) {
      while (ray_caster->nextId(idx)) {
        if (occupied_count < mp_->occupied_thr) {
          occupied_count++;
          continue;
        }
...
int SDFMap::countVisibleCells(const Vector3d& pos, const double& yaw, const vector<Vector3d>& cluster) {
  ...
  frontier_cam_->fromOdom2Cam(pos, odom_orient, camera_pose, camera_orient);
  int visib_num = 0;
  for (const auto& cell : cluster) {
    // Check if frontier cell is inside FOV
    if (!frontier_cam_->inFOV(camera_pose, cell, camera_orient)) continue;
...
void SDFMap::countVisibleCells(const Vector3d& pos, const Quaterniond& ori, const vector<Vector3d>& cluster, set<int>& res) {
  Eigen::Vector3i idx;
  Eigen::Vector3d camera_pose;
  Eigen::Quaterniond camera_orient;
  frontier_cam_->fromOdom2Cam(pos, ori, camera_pose, camera_orient);
  for (int i = 0; i < static_cast<int>(cluster.size()); i++) {
    const auto& cell = cluster[i];
    // Check if frontier cell is inside FOV
    if (!frontier_cam_->inFOV(camera_pose, cell, camera_orient)) continue;
...
void SDFMap::countVisibleCells(
    const Vector3d& pos, const Quaterniond& ori, const vector<Vector3d>& cluster, const vector<int>& mask, set<int>& res) {
  Eigen::Vector3i idx;
  Eigen::Vector3d camera_pose;
  Eigen::Quaterniond camera_orient;
  frontier_cam_->fromOdom2Cam(pos, ori, camera_pose, camera_orient);
...
bool SDFMap::getVisibility(const Vector3d& pos, const Vector3d& point) {
  ...
  frontier_cam_->fromOdom2Cam(pos, camera_pos);
  if (!frontier_cam_->inVisbleDepth(camera_pos, point)) return false;
...
bool SDFMap::getVisibility(const Vector3d& pos, const double& yaw, const Vector3d& point) {
  ...
  if (!frontier_cam_->inFOV(camera_pose, point, camera_orient)) return false;
