...
public:
  ...
  // Pass external caster points to prevent competition
  bool checkObstacleBetweenPoints(const Eigen::Vector3d& start, const Eigen::Vector3d& end, RayCaster* caster = nullptr);
  bool isGlobalMap_;
...
public:
  ...
  void countVisibleCells(const Vector3d& pos, const Quaterniond& ori, const vector<Vector3d>& cluster, set<int>& res);
  void countVisibleCells(
...
struct MapData {
  ...
  std::vector<Eigen::Vector3d> feature_map_;
  std::vector<Eigen::Vector3d> visual_feature_;
