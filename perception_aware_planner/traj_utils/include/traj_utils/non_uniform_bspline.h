...
public:
  ...
  void evaluateDeBoorTVec(const double t_in, std::vector<EIGENVEC>& vec) {
    vec.clear();
    double duration = getTimeSum();
    for (double t = 0.0; t < duration + 1e-3; t += t_in) {
      vec.emplace_back(evaluateDeBoorT(t));
    }
  }
  ...
  double getJerk();
  void getMeanAndMaxVel(double& mean_v, double& max_v, const double dur = -1.0);
  void getMeanAndMaxAcc(double& mean_a, double& max_a, const double dur = -1.0);
